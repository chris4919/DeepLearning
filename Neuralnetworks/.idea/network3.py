...
##load the MNIST data
def load_data_shared(filename='/Users/chris/Documents/Neuralnetworks/mnist.pkl.gz'):
...
class Network(object):
    ...
    def SGD(self,training_data,epochs,mini_batch_size,eta,validation_data,test_data,lmbda=0.0):
        ...
        num_validation_betches=size(validation_data)/mini_batch_size
        num_test_batches=size(test_data)/mini_batch_size

        #define the cost function symbolic gredients,and updates
        l2_norm_squared=sum([(layer.w**2).sum() for layer in self.layers])
        cost=self.layers[-1].cost(self)+0.5*lmbda*l2_norm_squared/num_training_batches
        grads=T.grad(cost,self.params)
        updates=[(param,param-eta*grad) for param,grad in zip(self.params,grads)]

        #define function to train a mini_batch and to compute the accuracy in validation and test mini_batches
        i=T.lscalar()
            ...
            for minibatch_index in xrange(num_training_batches):
                ...
                if(iteration+1)% num_training_batches==0:
                    validation_accuracy=np.mean(
                        [validate_mb_accuracy(j) for j in xrange(num_validation_betches)]
                    )
...
class ConvPoolLayer(object):
    ...
    def set_inpt(self,inpt,inpt_dropout,mini_batch_size):
        ...
        self.output_dropout=self.output #no drops in cov layers
